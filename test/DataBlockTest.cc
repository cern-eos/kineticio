...
SCENARIO("DataBlock integration test.", "[Data]")
{
  ...
  for(auto it = clusternames.begin(); it != clusternames.end(); it++) {
    REQUIRE(c.reset());
    auto cluster = kio::kio().cmap().getCluster(*it);

    GIVEN (*it + " and an empty block with create flag set.") {
      ...
      THEN("Illegal writes fail.") {
        ...
        REQUIRE_THROWS_AS(data.write(buf, cluster->limits().max_value_size + 1, 1), std::system_error);
      }
