...
int kinetic_help()
{
  ...
  fprintf(stdout, "             setting --security, --cluster, or --location shows only the selected subconfiguration\n");
  fprintf(stdout, "\n");
  ...
  fprintf(stdout, "             uploads the supplied local-json-file as current version for the selected\n");
...
void do_operation(Configuration& config, std::shared_ptr<kio::AdminClusterInterface> ac)
{
...
int main(int argc, char** argv)
{
  ...
  try {
    do_operation(config, kio::KineticIoFactory::makeAdminCluster(config.id.c_str()));
  } catch (std::exception& e) {
    fprintf(stdout, "Encountered Exception: %s\n", e.what());
    return EXIT_FAILURE;
  }
